<template>
  <component
      class="child-button flex items-center p-2 text-primary-900 rounded-lg dark:text-neutral-0 hover:bg-primary-100 dark:hover:bg-primary-700"
  ...
  <div v-else @click="toggleShowChildren" class="child-button flex items-center p-2 text-primary-900 rounded-lg dark:text-neutral-0 hover:bg-primary-100 dark:hover:bg-primary-700">
    <VrxIcon v-if="config.icon" :icon="config.icon" size="4"/>
    <span class="flex-1 whitespace-nowrap">{{ config.text }}</span>
